...
Connection(主机，端口，账户，密码)即可获取链接对象
...
```python
# 执行插入操作
pymysql库在执行对数据库进行修改操作时，需要通过连接对象的commit成员方法来进行确认的
# 设置自动提交
conn = Connection(
    host="localhost",      # 主机名（IP）
    port=3306,             # 端口
...
```python
# RDD：弹性分布式数据集
# RDD对象： pyspqrk在完成对多个数据的输入之后，都会得到一个RDD的类对象
```
...
rdd = sc.textFile("D:/Code/python/class/第15章/hello.txt")
print(rdd.collect())
...
import os
os.environ['PYSPARK_PYTHON'] = "D:/Anaconda/envs/pytorch/python.exe"
# 准备一个RDD
...
```python
# 功能：针对KV型（二元元组）RDD，自动按照key分组，根据聚合逻辑，完成组内数据（value）的聚合操作
# 接受一个处理函数，对数据进行两两计算
...
# 练习案例1：统计单词出现的次数
import os
os.environ['JAVA_HOME'] = "C:/Program Files/Java/jdk-22"  # 记得把地址改成自己的
os.environ['PYSPARK_PYTHON'] = "D:/Anaconda/envs/pytorch/python.exe"
# 1.构建执行环境入口对象
...
# 6.打印输出结果
print(rdd4.collect())  # [('itcast', 4), ('python', 6), ('itheima', 7), ('spark', 4), ('pyspark', 3)]
sc.stop()
```
...
# 对RDD的数据进行去重
rdd2 = rdd.distinct()
print(rdd2.collect())  # [1, 3, 5, 7, 8, 9, 10]
```

##### （9）sortBy方法

```python
# 功能：对RDD数据进行排序，基于指定排序依据
rdd.sortBy(func, ascending=False, numPartitions=1)
func: 告知按照rdd中的那个数据进行排序，比如：lambda x: x[1]表示按照rdd中的第二列元素进行排序
ascending: True升序， False降序
numParttitions:用多少进行区分排序
# 对（6）中练习案例的结果进行排序
rdd5 = rdd4.sortBy(lambda x: x[1], ascending=False, numPartitions=1)
print(rdd5.collect())  # [('itheima', 7), ('python', 6), ('itcast', 4), ('spark', 4), ('pyspark', 3)]
```
...
# TODO 需求1：城市销售额排名
# 1.1 读取文件得到RDD
file_rdd = sc.textFile("D:/Code/python/class/第15章/orders.txt")
# 1.2 取出一个json字符串
...
print("需求1的结果为：", result1_rdd.collect())

# TODO 需求2： 全部城市有哪些商品类别在售卖
# 2.1 取出全部商品的类别并对商品类别进行去重
result2_rdd = dict_rdd.map(lambda x: x['category']).distinct()
print("需求2的结果为：", result2_rdd.collect())
...
print("需求3的结果为：", result3_rdd.collect())
```
...
```python
# 功能：统计rdd内有多少条数据，返回值为数字
num_count = rdd.count()  # 5
```
...
# 输出的结果是一个文件夹，有几个分区就有几个结果文件
# 修改RDD分区：两种方法，见下面的1.和2.
conf = SparkConf().setMaster("local[*]").setAppName("test_spark")
conf.set("spark.default.parallelism", "1")  # 1.
sc = SparkContext(conf=conf)

# 准备RDD
rdd1 = sc.parallelize([1, 2, 3, 4, 5], numSlices=1)  # 2.
rdd2 = sc.parallelize([("Hello", 3), ("Spark", 5), ("Hi", 7)])
rdd3 = sc.parallelize([[1, 3, 5], [6, 7, 9], [11, 13, 11]])

# 输出到文件中
rdd1.saveAsTextFile("D:/Code/python/class/第15章/output1")
rdd2.saveAsTextFile("D:/Code/python/class/第15章/output2")
rdd3.saveAsTextFile("D:/Code/python/class/第15章/output3")
```
...
# 读取文件转换为RDD
file_rdd = sc.textFile("D:/Code/python/class/第15章/search_log.txt")
...
# TODO 需求3： 统计黑马程序员关键词在什么时间段被搜索的最多
...
file_rdd.map(lambda x: x.split("\t")).\
    ...
    saveAsTextFile("D:/Code/python/class/第15章/output_json")
```
...
# 简单闭包：防止函数中的变量被篡改，无需定义全局变量，可以通过函数、持续的访问、修改某个值
def outer(logo):
...
```python
# 使用nonlocal关键字修改外部函数的值：修改外部函数的变量值
def outer(num1):
...
def account_create(initial_account=0):
    ...
    def atm(num, deposit=True):
        ...
        if deposit:
            initial_account += num
            print(f"存款:+{num}，账户余额为：{initial_account}")
        else:
            initial_account -=num
            print(f"取款:-{num}，账户余额为：{initial_account}")
...
```python
# 功能：在不破坏原有代码和功能的前提下，为目标函数增加新功能
# 创建一个闭包函数，再逼包函数内部调用目标函数
def outer(func):
    def inner():
        print("我睡觉了")
        func()
...
```python
# 设计模式就是一种编程套路
# 单例设计模式：对一个类，只获取其唯一的类实例对象，持续复用它
# 优点：1.节省内存， 2.节省创建对象的开销
```
...
```python
# 进程：一个程序运行在系统之上，便称这个程序为一个运行进程
# 线程：一个进程可以开启多个线程，执行不同的工作
# 并行执行：同一时间做不同的工作
```
...
# 多线程编程
# threading模块的使用
thread_obj=threading.Tread(target=func)  # 创建编程对象
...
```python
# socket:进程之间进行网络通信的媒介
# socket服务端：等待其他进程连接，可接受发来的消息，可以回复消息
# socket客户端：主动连接服务端
```
...
'''accept()方法是阻塞方法，等待客户端的连接，如果没有连接就卡在这一行不往下执行'''
conn, address = socket_server.accept()  # 接受二元元组内的两个元素
print(f"接收到了客户端的连接，客户端的信息是：{address}")

while True:
    # 接受客户端信息，要使用客户端和服务端的本次连接对象，而非socket_server对象
    # recv接受的参数是缓冲区大小，一般给到1024即可
    # recv方法的返回值是一个字节数组也就是bytes对象，不是字符串，可以通过decode方法通过UTF-8编码，将字符数组转换为字符串对象
    data = conn.recv(1024).decode("UTF-8")
    print(f"客户端发来的消息是：{data}")

    # 发送回复消息
    msg = input("请输入您要和客户端回复的消息")  # encode可以将字符串编码为字节数组对象
    if msg == 'exit':
        break
...
while True:
    ...
    # 接受返回消息
    recv_data = socket_client.recv(1024)  # 1024是缓冲区的大小，一般1024即可，recv方法是阻塞的
...
```python
# 正则表达式：使用单个字符串来描述、匹配某个句子规则的字符串
# python正则表达式，基于re模块中的三个基础方法来做正则匹配
...
print(result2)               # <re.Match object; span=(1, 7), match='python'>

# findall 搜索全部匹配
result3 = re.findall("python", s)
print(result3)               # ['python', 'python', 'python']
```
...
result = re.findall(r'\W', s)  # r使''里的转义字符无效
print(result)  # [' ', '@', '@', ' ', '!', '!', ' ', '#', '#']
```
...
result = re.findall(r'\W', s)  # r使''里的转义字符无效，作为普通字符来使用
print(result)

# 匹配账号，只能由字母和数字组成，长度限制6-10位
r = '^[0-9a-zA-Z]{6,10}$'  # 从开头匹配到结尾
s = '123456123'
print(re.findall(r, s))
# 匹配QQ号，要求纯数字，长度5-10，第一位不为0
r = '^[1-9][0-9]{4,10}$'
s = '12345678'
print(re.findall(r, s))
# 匹配邮箱号，只允许qq、163、gmail这三种邮箱地址
...
def test_os():
    # os模块的3个基础方法
    print(os.listdir("D:/test"))      # 列出test文件夹中的所有文件
    print(os.path.isdir("D:/test/a")) # 判断a是否是一个文件夹 True/False
    print(os.path.exists("D:/test"))  # 判断指定路径是否存在  True/False
    
def get_files_recursion_from_dir(path):
    """
    从指定文件夹中使用递归方式，获取全部的文件夹列表
    :param path: 被判断的文件夹
    :return: list包含全部的文件，如果目录不存在就返回一个空list
    """
    file_list = []
